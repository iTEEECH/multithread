...
class _HomePageState extends State<HomePage> {
  ...
  void initState() {
    super.initState();
    _startTimer();
    _future = _fetchData(kIsWeb ? 'data.json' : 'assets/data.json');
  }
  ...
  Future<List<Beer>> _fetchData(String path) async {
    try {
      ...
      return await _parserWithIsolates(response);
    } catch (error) {
  ...
  Future<List<Beer>> _parserWithIsolates(String response) async {
    final ReceivePort receivePort = ReceivePort();
    try {
      final ReceivePort receivePort = ReceivePort();
      await Isolate.spawn(_parser, [receivePort.sendPort, response]);
  ...
  static Future<void> _parser(List<Object> args) async {
    final SendPort sendPort = args[0] as SendPort;
    final String response = args[1] as String;
          ...
          if (snapshot.hasData) {
            return ListView.builder(
              ...
              itemBuilder: (_, int index) {
                ...
                return ListTile(
                  title: Text('Name ${beer?.name} - Brand ${beer?.brand}'),
                  subtitle: Text('Type ${beer?.type}'),
                  trailing: Text('Country ${beer?.country}'),
                );
              },
            );
          }
